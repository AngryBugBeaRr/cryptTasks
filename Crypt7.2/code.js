function crypt (btnNumber) {
    ...
    switch (btnNumber) {
        ...
        case 3:
            ...
            break;
        case 4:
            result = document.getElementById('1').innerHTML.substr(11, rusTextLength + 11);
            for (let i = 0; i < result.length; i++) {
                let help = 1;
                for (let j = i + 1; j < rusTextLength; j++) {
                    if (result[i] === result[j]) {
                        help++;
                        result = result.replace(result[j], '');
                    }
                }
                index += Math.pow(help, 2) / Math.pow(rusTextLength, 2);
                help = 0;
            }
        ...
        case 5:
            ...
            break;
        case 6:
            result = document.getElementById('3').innerHTML.substr(11, armTextLength + 11);
            for (let i = 0; i < result.length; i++) {
                let help = 1;
                for (let j = i + 1; j < result.length; j++) {
                    if (result[i] === result[j]) {
                ...
                index += Math.pow(help, 2) / Math.pow(armTextLength, 2);
                help = 0;
            }
